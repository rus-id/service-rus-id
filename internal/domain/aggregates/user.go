...
func NewUser(
	...
	name *valuetypes.Name,
	phone *valuetypes.Phone,
	registrationDate time.Time) (UserAggregate, error) {
	if id == nil {
	...
	rating, _ := valuetypes.NewRating(0, 0)
	tolerances := make(map[valuetypes.UserID]*valuetypes.Tolerance)
	state := valuetypes.UserStateActive
...
func LoadUser(snapshot snapshots.UserSnapshot) *User {
	return nil
}
...
func (u *User) GetSnapshot() *snapshots.UserSnapshot {
	return nil
}
...
func (u *User) GrantFullAccess(userID valuetypes.UserID) {
	...
	newTolerances = newTolerances.AddFullAccess()
	u.tolerances[userID] = newTolerances
}
